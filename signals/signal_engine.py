...
class SignalEngine:
    ...
    @classmethod
    def _timeframe_context(cls, candles: List[dict]) -> dict:
        if len(candles) < EMA_SLOW_PERIOD:
            return {"trend": "unknown", "adx": 0.0, "ema_fast": 0.0, "ema_slow": 0.0}
        closes = [c["close"] for c in candles]
        ema_fast = cls._ema(closes[-EMA_SLOW_PERIOD:], EMA_FAST_PERIOD)
        ema_slow = cls._ema(closes[-EMA_SLOW_PERIOD:], EMA_SLOW_PERIOD)
        adx = cls._adx(candles, ADX_PERIOD)
        start = closes[-EMA_SLOW_PERIOD]
        change = (closes[-1] - start) / start if start else 0.0
        close = closes[-1]
        if adx >= MTF_MIN_ADX and change >= MTF_MIN_TREND_PCT and ema_fast > ema_slow and close > ema_fast:
            trend = "trend_up"
        elif adx >= MTF_MIN_ADX and change <= -MTF_MIN_TREND_PCT and ema_fast < ema_slow and close < ema_fast:
            trend = "trend_down"
        else:
            trend = "range"
        return {"trend": trend, "adx": adx, "ema_fast": ema_fast, "ema_slow": ema_slow}

    def _analyze(self, symbol: str) -> None:
        candles = list(self.candles[symbol])
        minimum = max(EMA_SLOW_PERIOD, DONCHIAN_ENTRY_PERIOD + 1, DONCHIAN_EXIT_PERIOD + 1, ATR_PERIOD + 1, ADX_PERIOD + 1)
        ...
        if regime == MarketRegime.TREND_UP:
            ...
            mtf_aligned = trend15 == "trend_down" and trend30 == "trend_down"
        else:
            mtf_aligned = False
        ...
        if MTF_ENABLED and not mtf_ok:
            ...
            reason = "entry_quality_failed"

        self.market_analysis[symbol] = MarketAnalysis(
            gen_trend=regime_value,
            candle_trend="up" if close > candles[-1]["open"] else "down" if close < candles[-1]["open"] else "flat",
            trend_strength=adx, volatility_pct=atr_pct, price_change_pct=price_change_pct,
            price_range_pct=(highest_entry - lowest_entry) / close if close else 0.0,
            is_high_volatility=atr_pct > MAX_ATR_PCT, should_trade=should_trade,
            trade_reason=reason, confidence=confidence, trend_age=age,
