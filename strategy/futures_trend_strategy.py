...
class FuturesTrendStrategy:
    """Mechanical trend-following entry model.

    Entry = healthy directional regime + Donchian breakout. This follows the
    well-established trend-following/Turtle family rather than attempting to
    predict a bottom or top.
    """
    ...
    def evaluate(self, candles, analysis):
        ...
        if REQUIRE_ADX_CONFIRMATION and analysis.adx < MIN_ADX:
            return None
        if REQUIRE_VOLUME_CONFIRMATION and analysis.volume_ratio < 1.0:
            return None
        ...
        if analysis.gen_trend == "trend_up" and analysis.entry_breakout and ALLOW_LONG:
            if REQUIRE_EMA_ALIGNMENT and analysis.ema_fast <= analysis.ema_slow:
                return None
            return Signal(SignalType.LONG, size_factor=1.0)

        if analysis.gen_trend == "trend_down" and analysis.entry_breakout and ALLOW_SHORT:
            if REQUIRE_EMA_ALIGNMENT and analysis.ema_fast >= analysis.ema_slow:
                return None
            return Signal(SignalType.SHORT, size_factor=1.0)
