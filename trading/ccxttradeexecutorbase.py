...
from config.settings import (
    ...
    TRAIL_MAX_DISTANCE_PCT,
    STRONG_ADX,
    EXTREME_ADX,
    BASE_LEVERAGE,
    STRONG_TREND_LEVERAGE,
    EXTREME_TREND_LEVERAGE,
    MAX_LEVERAGE,
    EXIT_ON_OPPOSITE_BREAKOUT,
    EXIT_ON_EMA_REVERSAL,
...
class CCXTTradeExecutorBase(ABC):
    """Paper/live-neutral futures risk engine.

    Leverage is a regime control, not a recovery/progression mechanism.
    Position size is calculated from account risk and the actual stop distance.
    """
    ...
    def _calculate_stop(self, side, entry, analysis, candles):
        ...
        if atr <= 0:
            return None
        swing_lookback = min(len(candles), max(1, len(candles)))
        recent = candles[-swing_lookback:]
        if side == "long":
            swing = min(c["low"] for c in recent)
            atr_stop = entry - atr * STOP_ATR_MULTIPLIER
            structure_stop = swing - atr * STOP_SWING_BUFFER_ATR
    ...
    def _open_position(self, symbol, side, price, analysis, candles, candle_id, tick_id):
        ...
        if stop is None:
            print(f"[RISK] Skip {symbol}: calculated stop exceeds configured risk envelope")
            return
        ...
        print(
            f"[EXECUTOR] OPEN {side.upper()} trade={self.position.trade_id} "
            f"{symbol} price={price:.6f} lev={leverage:.1f}x "
            f"stop={stop:.6f} risk={risk_amount:.2f}"
        )
    ...
    def _close_position(self, price, reason, tick_id=None, candle_id=None):
        ...
        print(
            f"[EXECUTOR] CLOSE {pos.side.upper()} trade={pos.trade_id} "
            f"price={price:.6f} pnl={net:.4f} ({pos.pnl_pct(price):.2f}%) reason={reason} "
